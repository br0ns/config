...
scratchpadSpawnActionCustom :: String -- ^ Command to spawn a program with resource \"scratchpad\"
                                 -> String
                                 -> X ()
scratchpadSpawnActionCustom suffix c = namedScratchpadAction [NS name c (resource =? name) nonFloating] name
  where name = "scratchpad-" ++ suffix

-- factored out since this is common to both the ManageHook and the action
scratchpadQuery :: Query Bool
scratchpadQuery = fmap (isPrefixOf "scratchpad-") resource
